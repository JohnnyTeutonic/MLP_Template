...
class neural_network_multiclass {

public:
	neural_network_multiclass(unsigned int _n_inputs,
...
private:
	...
	double softmax_gradient(double z);
	double relu_prime(const double z);
	double sigmoid_prime(const double z, bool first);
	void calculate_gradients(doubleVector& delta, const doubleVector& z, const doubleVector& x, const doubleVector& y);
